...
dn.set_gpu(0)
net = dn.load_net("cfg/yolo-obj.cfg", os.path.join(DARKNET_PATH,
                                                   "backup/yolo-obj_900.weights"), 0)
meta_data_net = dn.load_meta("data/obj.data")


test_images_list_file = open(os.path.join(DARKNET_PATH, "data/train.txt"), "r")
...
#     gt_boxes = get_GT_boxes(os.path.join(
#         DARKNET_PATH, (img.strip()[:-4] + '.xml')))
#     detections = get_yolo_detections(img, net, meta_data_net)
#     im_path = os.path.join(DARKNET_PATH, img.strip())
